...
const DiagnosticsLite = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 max-w-md mx-auto overflow-hidden">
      <div className="flex-1 p-4 sm:p-6 space-y-4 sm:space-y-6">
        <div className="text-center">
          <h1 className="text-2xl sm:text-3xl font-medium mb-1 mx-16">
        ...
        <Card className="relative">
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <h3 className="text-lg sm:text-xl font-semibold mb-2">Summary</h3>
            ...
            <div className="overflow-x-auto pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <div className="flex space-x-3 px-1 py-2 w-max">
                {insights.map((insight: HealthInsight) => (
                  <Card
                    key={insight.id}
                    className="relative overflow-hidden flex-shrink-0 w-[85%] max-w-[280px] shadow-sm hover:shadow transition-shadow"
                  >
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                    ...
                    <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-orange-300 to-orange-500"></div>
                  </Card>
                ))}
              </div>
            </div>
              ...
              {insights.map((_, index) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    index === 0 ? 'w-4 bg-orange-500' : 'w-1.5 bg-gray-300'
                  }`}
                ></div>
              ))}
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h3 className="text-lg sm:text-xl font-semibold">Results</h3>
          ...
          <div className="space-y-2">
            {results.map((result: HealthMetric) => (
              <Card
                key={result.id}
                className={cn(
                  'relative overflow-hidden active:bg-gray-100 transition-colors cursor-pointer',
                )}
                onClick={() =>
                  setActiveInsight(
